...
import { Camera, FileUp, Search, CheckCircle2, AlertTriangle, Clock, Check, CreditCard, Calendar as CalendarIcon, X, Receipt, Loader2, Save, Fingerprint, FileDown } from "lucide-react";
    ...
    <div className="p-3 space-y-4 h-full flex flex-col animate-in fade-in slide-in-from-right-4 duration-500">
      <header className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          >
            ...
            <input type="file" ref={fileInputRef} className="hidden" onChange={handleScan} accept="image/*,application/pdf" />
          </button>
        </div>

        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={14} />
          <Input placeholder="Search..." className="pl-9 bg-gray-100/50 border-0 h-10 text-sm rounded-xl" />
        </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-2 pb-4 no-scrollbar">
        {invoices.filter(inv => activeTab === 'all' || inv.status === activeTab).map((inv) => (
          <div key={inv.id} onClick={() => setSelectedInvoice(inv as any)} className="bg-white p-3 rounded-xl shadow-sm border border-gray-100 active:scale-[0.98] transition-all cursor-pointer">
            <div className="flex justify-between items-start mb-1">
              <div className="flex-1 min-w-0 mr-2">
                <div className="flex items-center gap-1.5 mb-0.5">
                   <h3 className="font-bold text-gray-900 text-sm truncate">{inv.vendor}</h3>
                   {inv.irn && <Fingerprint size={12} className="text-emerald-500" />}
                </div>
                <p className="text-[10px] text-gray-500 font-medium">{inv.id} • {inv.date}</p>
                <div className="flex gap-1 mt-0.5 overflow-hidden">
                  <span className="text-[9px] text-primary/70 bg-primary/5 px-1 rounded uppercase font-bold tracking-tight">GSTIN: {inv.gstin}</span>
                </div>
              </div>
          ...
          {selectedInvoice && (
            <div className="p-5 space-y-5 overflow-y-auto no-scrollbar">
              <DrawerHeader className="p-0 text-left">
                ...
                <DrawerDescription className="text-xs text-gray-500">GSTIN: {selectedInvoice.gstin}</DrawerDescription>
              </DrawerHeader>
              {selectedInvoice.irn && (
